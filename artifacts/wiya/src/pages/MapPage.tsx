...
export default function MapPage() {
  ...
  const filteredWilayas = query
    ? WILAYAS_DATA.filter((w) => w.name.toLowerCase().includes(query.toLowerCase()) || w.nameAr.includes(query))
    : [];
    ...
    <div className="bg-[#0f3d22] min-h-screen pb-20 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center px-2">
        <div className="w-full bg-[#1a5c36]/40 rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
          <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full" style={{ background: "transparent" }}>
            <rect width={SVG_W} height={SVG_H} fill="#0d3320" rx="16" />
            <path d={ALGERIA_PATH} fill="#1e7a44" stroke="#2a9955" strokeWidth="1.5" strokeLinejoin="round" />
            <path d={ALGERIA_PATH} fill="url(#sahara)" opacity="0.4" />
            <defs>
            ...
            {WILAYAS_DATA.map((wilaya) => {
              ...
              return (
                <g key={wilaya.code} onClick={() => setSelected(isSelected ? null : wilaya)} style={{ cursor: "pointer" }}>
                  {isSelected && <circle cx={x} cy={y} r={14} fill="none" stroke="#ff6b35" strokeWidth="1.5" opacity="0.5" />}
                  <circle cx={x} cy={y} r={r} fill={fill} stroke={stroke} strokeWidth={isSelected ? 2 : hasListings ? 1 : 0.8} filter={isSelected || hasListings ? "url(#glow)" : undefined} />
                  {hasListings && !isSelected && (
                    <>
                      <circle cx={x + 7} cy={y - 7} r={6} fill="#1B6B3A" stroke="#E8C84A" strokeWidth="1" />
                      <text x={x + 7} y={y - 7} textAnchor="middle" dominantBaseline="central" fontSize="6" fontWeight="bold" fill="white">{count}</text>
                    </>
                  )}
                  {(hasListings || isSelected) && (
                    <text x={x} y={y + r + 7} textAnchor="middle" fontSize={isSelected ? "7" : "6"} fontWeight="bold" fill={isSelected ? "#ff6b35" : "#E8C84A"} opacity={isSelected ? 1 : 0.85}>
                      {wilaya.code}
                    </text>
                  )}
                </g>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "100%", opacity: 0 }} transition={{ type: "spring", damping: 28, stiffness: 300 }} className="fixed bottom-16 left-0 right-0 max-w-[430px] mx-auto z-50">
