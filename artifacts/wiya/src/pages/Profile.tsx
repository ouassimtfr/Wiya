...
function UserAvatar({ name, avatarUrl, size = 72 }: { name: string; avatarUrl?: string; size?: number }) {
 ...
 if (avatarUrl && !avatarUrl.includes("dicebear") && !avatarUrl.includes("api.dicebear")) {
   return (
     <img
       src={avatarUrl}
       alt={name}
       style={{ width: size, height: size }}
       className="rounded-full bg-white/20 border-2 border-white/40 shadow-lg object-cover"
     />
   );
 }
 ...
 return (
   <div
     style={{ width: size, height: size }}
     className="rounded-full bg-white/30 border-2 border-white/40 shadow-lg flex items-center justify-center"
   >
...
export default function ProfilePage() {
 ...
 const menuItems = [
   ...
   { icon: Settings, label: "Paramètres", action: () => setShowSettings(true), badge: 0 },
   ...
   <div className="bg-[#F4F6F5] min-h-screen pb-20">
     <div className="bg-[#1B6B3A] pt-12 pb-5 px-4 relative overflow-hidden">
       ...
       <div className="relative flex items-center gap-4">
         ...
         <button onClick={() => setShowSettings(true)} className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center"><Settings className="w-4 h-4 text-white" /></button>
       </div>
     ...
     <AnimatePresence>
       {showSettings && (
         <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-end" onClick={() => setShowSettings(false)}>
           <motion.div initial={{ y: 300 }} animate={{ y: 0 }} exit={{ y: 300 }} onClick={(e) => e.stopPropagation()} className="bg-white w-full rounded-t-3xl p-5 space-y-4">
         ...
         <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-end" onClick={() => setShowSupport(false)}>
           <motion.div initial={{ y: 300 }} animate={{ y: 0 }} exit={{ y: 300 }} onClick={(e) => e.stopPropagation()} className="bg-white w-full rounded-t-3xl p-5 space-y-4">
             ...
             <div className="space-y-3">
               <a href="https://wa.me/213XXXXXXXXX" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-green-50 rounded-2xl">
