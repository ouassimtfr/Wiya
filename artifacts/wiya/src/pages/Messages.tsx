...
export default function ChatPage() {
  ...
  // Si pas de user, on redirige vers login ou affiche un message
  if (!user) {
    return (
  ...
  // Si conversation introuvable, on affiche un état d'attente plutôt qu'une erreur bloquante
  if (!conversation) {
    return (
      <div className="bg-[#F4F6F5] min-h-screen flex flex-col items-center justify-center p-4">
        <p className="text-gray-500 mb-4">Chargement de la conversation...</p>
        <button 
    ...
    <div className="bg-[#F4F6F5] min-h-screen flex flex-col pb-6">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">Avec {conversation.otherUser?.name || "Utilisateur"}</p>
        </div>
      </div>
