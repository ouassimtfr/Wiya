...
export default function Home() {
  ...
  return (
    <div className="bg-[#F4F6F5] min-h-screen pb-20">
      {/* HEADER AVEC BOUTON WILAYA FONCTIONNEL */}
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <button 
              ...
              className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-3 py-1 rounded-full border border-white/10"
            >
      ...
      <AnimatePresence>
        {showCategoryPicker && (
          <div className="fixed inset-0 bg-black/40 z-[9999] flex items-end" onClick={() => setShowCategoryPicker(false)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-[430px] mx-auto rounded-t-3xl flex flex-col overflow-hidden" style={{ height: "85vh" }}>
              <div className="px-6 pt-6"><div className="w-10 h-1 bg-gray-200 rounded-full mx-auto mb-6" /></div>
              <div className="flex-1 overflow-y-auto px-6 pb-20 grid grid-cols-4 gap-3">
                {CATEGORIES.map((cat) => (
        ...
        {showWilayaPicker && (
          <div className="fixed inset-0 bg-black/40 z-[9999] flex items-end" onClick={() => setShowWilayaPicker(false)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-[430px] mx-auto rounded-t-3xl flex flex-col overflow-hidden" style={{ height: "85vh" }}>
              <div className="p-4"><input autoFocus type="text" value={wilayaSearch} onChange={(e) => setWilayaSearch(e.target.value)} placeholder={t("searchWilaya")} className="w-full bg-gray-100 p-3 rounded-xl outline-none text-sm" /></div>
              <div className="flex-1 overflow-y-auto px-4 pb-6 grid grid-cols-2 gap-2">
                {filteredWilayas.map((w) => (
                  <button key={w} onClick={() => { setActiveWilaya(w); setShowWilayaPicker(false); }} className={`p-3 rounded-xl text-sm ${activeWilaya === w ? "bg-[#1B6B3A] text-white" : "bg-gray-50"}`}>{w}</button>
                ))}
              </div>
            </motion.div>
          </div>
