...
export default function SearchPage() {
  ...
  const fetchResults = async () => {
    ...
    if (selectedWilaya) q = q.eq("wilaya", selectedWilaya);
        ...
        {showFilters && (
          ...
          >
            <div className="px-4 py-4 space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">{t("wilaya")}</label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">{t("priceRange")}</label>
                <div className="flex gap-2">
                  <input type="number" placeholder={t("minPrice")} value={minPrice} onChange={(e) => setMinPrice(e.target.value)} className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none" />
                  <input type="number" placeholder={t("maxPrice")} value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none" />
                </div>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">{t("sort")}</label>
                <div className="flex gap-2">
                  {(["newest", "priceAsc", "priceDesc"] as SortOption[]).map((s) => (
                    <button key={s} onClick={() => setSort(s)} className={`flex-1 py-2 rounded-xl text-xs font-semibold transition-colors ${sort === s ? "bg-[#1B6B3A] text-white" : "bg-gray-100 text-gray-600"}`}>
                      {t(s)}
                    </button>
                  ))}
                </div>
